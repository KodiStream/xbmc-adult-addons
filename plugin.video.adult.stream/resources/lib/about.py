...
#sLibrary = xbmc.translatePath("special://home/addons/plugin.video.adult.stream").decode("utf-8")
...
class cAbout:       
    ...
    def getUpdate(self):
        ...
        if (service_time):
            ...
            if (time_now - time_service > time_sleep):
                ...
                sUrl = 'https://api.github.com/repos/KodiStream/xbmc-adult-addons/releases/latest'
            ...
            if (time_now - time_service > time_sleep):
                ...
                if result:
                    ...
                    if (sDown != 0):
                        ...
                        #cConfig().showInfo('Adukt Stream', 'Fichier a jour')
                        addons.setSetting('service_time', str(datetime.datetime.now()))
                        addons.setSetting('home_update', str('false'))
    ...
    def getRootPath_old(self, folder):
        sMath = cConfig().getAddonPath().replace('plugin.video.adult.stream', '').decode("utf-8")
    ...
    def resultGit(self):
        ...
        try: 
            sUrl = 'https://raw.githubusercontent.com/KodiStream/xbmc-adult-addons/master/sites.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request()
            result = json.loads(sHtmlContent)
            
            sUrl = 'https://raw.githubusercontent.com/KodiStream/xbmc-adult-addons/master/hosts.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request()
            result += json.loads(sHtmlContent)

            sUrl = 'https://raw.githubusercontent.com/KodiStream/xbmc-adult-addons/master/scrapers.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request()
            result += json.loads(sHtmlContent)
    ...
    def checkdownload(self):
        ...
        if dialogs.VSyesno("Êtes-vous sûr ?"):
            ...
            #result = self.resultGit()
            sUrl = 'https://api.github.com/repos/KodiStream/xbmc-adult-addons/compare/%s...%s' % (service_version, service_futur)
            #pour test
            #sUrl = 'https://api.github.com/repos/KodiStream/xbmc-adult-addons/compare/0.6.3...0.6.31'
            ...
            if result: 
                ...
                for i in result['files']:
                    ...
                    if 'plugin.video.adult.stream' in i['filename']: 
                ...
                listitem = xbmcgui.ListItem(label = "Adult Stream", label2 = sContent)
                icon = "special://home/addons/plugin.video.adult.stream/resources/art/update.png"
    ...
    def TextBoxes(self, anounce):
        ...
        xbmc.sleep(100)
        # set heading
        win.getControl(1).setLabel("Adult Stream mise à jour")
