...
#
# A basic JS interpreter in python, made for the Kodi addon Adult Stream https://github.com/KodiStream/xbmc-adult-addons
#
#*************************************************************************
...
class JsParser(object):
    ...
    def VarParser(self, vars, allow_recursion, variable, op, JScode):
        ...
        if len(op) == 2:
            ...
            # a+=1 form
                ...
                if isinstance(r, (str)):
                    self.VarManage(allow_recursion, vars,
                                   variable, '"' + r + '"')
                else:
                    self.VarManage(allow_recursion, vars, variable, str(r))
    ...
    def evalJS(self, JScode, vars, allow_recursion):
        ...
        while (len(JScode) > 0):
            ...
            raise Exception(str(allow_recursion) +
                            " : Can't Eval chain : " + JScode)

        InterpretedCode2 = InterpretedCode.GetBuffer()

        if DEBUG:
            out(str(allow_recursion) + ' : Evalue > ' +
                Ustr(InterpretedCode2) + " type " + Ustr(type(InterpretedCode2)))
            out('-------------')
    ...
    def SetVar(self, var, variable, value, i=None):
        ...
        # print 'Setvar Variable =' + variable + ' value=' + str(value) + '
        # index=' + str(i)
    ...
    def VarManage(self, allow_recursion, vars, name, value=None):
        ...
        m = re.search(r'^\({0,1}([\w]+)\){0,1}\[(.+?)\]$',
                      name, re.DOTALL | re.UNICODE)
        if m:
            name = m.group(1)
            index = m.group(2)
    ...
    def Parse(self, JScode, vars, allow_recursion=MAX_RECURSION):
        ...
        while (True):
            ...
            if m:
                ...
                self.SetVar(self.HackVars, m.group(
                    1), self.GetVar(vars, m.group(2)))
                continue
...
class Basic(object):
    ...
    def eval(self, arg):
        if DEBUG:
            out('To eval >' + arg)
        r = self._JSParser.Parse(RemoveGuil(
            arg), self._JSParser.FastEval_vars, self._JSParser.FastEval_recur)
        return r
